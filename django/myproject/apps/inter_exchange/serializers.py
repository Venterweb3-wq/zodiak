...
class ActivateInvestmentSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        # Fetch the active config for inter_exchange
        config = InvestmentConfig.objects.filter(strategy='inter_exchange', is_active=True).first()
        if not config:
...
class CreateWithdrawalRequestSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        REDIS_WITHDRAWAL_CHANNEL = 'withdrawals_inter_exchange'

        msg = {
            "type": "withdrawal_request_inter_exchange",
            "app_name": "inter_exchange",
