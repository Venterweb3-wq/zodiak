...
class InvestmentAccount(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='investment_accounts_inter_exchange')
    ...
    def __str__(self):
        return f"InvestmentAccount (Inter Exchange) {self.user.username} - {self.get_network_display()} — {self.balance} USDT"
...
class TemporaryWallet(models.Model):
    ...
    user = models.ForeignKey(
        ...
        related_name='temp_wallets_inter_exchange'
    )
...
class InvestmentDeposit(models.Model):
    account = models.ForeignKey(InvestmentAccount, on_delete=models.CASCADE, related_name='deposits_inter_exchange')
    ...
    def __str__(self):
        return f"Deposit of {self.amount} to {self.account.user.username} (Inter Exchange) - TX: {self.tx_hash[:10]}..."


class DailyPayout(models.Model):
    account = models.ForeignKey(InvestmentAccount, on_delete=models.CASCADE, related_name='daily_payouts_inter_exchange')
    ...
    def __str__(self):
        ...
        return f"Daily Payout {self.date} for {self.account.user.username} (Inter Exchange) - {self.amount} USDT ({status})"
...
class InvestmentConfig(models.Model):
    strategy = models.CharField(
        max_length=64,
        unique=True,
        default='inter_exchange',
        help_text="Название стратегии (например, inter_exchange)"
    )
