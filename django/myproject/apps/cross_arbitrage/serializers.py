...
class ActivateInvestmentSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        # Fetch the active config for cross_arbitrage
        config = InvestmentConfig.objects.filter(strategy='cross_arbitrage', is_active=True).first()
        if not config:
...
class CreateWithdrawalRequestSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        REDIS_WITHDRAWAL_CHANNEL = 'withdrawals_cross_arbitrage'

        msg = {
            "type": "withdrawal_request_cross_arbitrage",
            "app_name": "cross_arbitrage",
