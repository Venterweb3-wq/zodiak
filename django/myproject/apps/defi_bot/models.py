...
class InvestmentAccount(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='investment_accounts_defi_bot')
    ...
    def __str__(self):
        return f"InvestmentAccount (DeFi Bot) {self.user.username} - {self.get_network_display()} — {self.balance} USDT"
...
class TemporaryWallet(models.Model):
    ...
    user = models.ForeignKey(
        ...
        related_name='temp_wallets_defi_bot'
    )
...
class InvestmentDeposit(models.Model):
    account = models.ForeignKey(InvestmentAccount, on_delete=models.CASCADE, related_name='deposits_defi_bot')
    ...
    def __str__(self):
        return f"Deposit of {self.amount} to {self.account.user.username} (DeFi Bot) - TX: {self.tx_hash[:10]}..."


class DailyPayout(models.Model):
    account = models.ForeignKey(InvestmentAccount, on_delete=models.CASCADE, related_name='daily_payouts_defi_bot')
    ...
    def __str__(self):
        ...
        return f"Daily Payout {self.date} for {self.account.user.username} (DeFi Bot) - {self.amount} USDT ({status})"
...
class InvestmentConfig(models.Model):
    strategy = models.CharField(
        ...
        unique=True,
        default='defi_bot',
        help_text="Название стратегии (например, defi_bot)"
    )
