...
class ActivateInvestmentSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        # Fetch the active config for spot_scalping
        config = InvestmentConfig.objects.filter(strategy='spot_scalping', is_active=True).first()
        if not config:
...
class CreateWithdrawalRequestSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        REDIS_WITHDRAWAL_CHANNEL = 'withdrawals_spot_scalping'

        msg = {
            "type": "withdrawal_request_spot_scalping",
            "app_name": "spot_scalping",
