...
class InvestmentAccount(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='investment_accounts_spot_scalping')
    ...
    def __str__(self):
        return f"InvestmentAccount (Spot Scalping) {self.user.username} - {self.get_network_display()} — {self.balance} USDT"
...
class TemporaryWallet(models.Model):
    ...
    user = models.ForeignKey(
        ...
        related_name='temp_wallets_spot_scalping'
    )
...
class InvestmentDeposit(models.Model):
    account = models.ForeignKey(InvestmentAccount, on_delete=models.CASCADE, related_name='deposits_spot_scalping')
    ...
    def __str__(self):
        return f"Deposit of {self.amount} to {self.account.user.username} (Spot Scalping) - TX: {self.tx_hash[:10]}..."


class DailyPayout(models.Model):
    account = models.ForeignKey(InvestmentAccount, on_delete=models.CASCADE, related_name='daily_payouts_spot_scalping')
    ...
    def __str__(self):
        ...
        return f"Daily Payout {self.date} for {self.account.user.username} (Spot Scalping) - {self.amount} USDT ({status})"
...
class InvestmentConfig(models.Model):
    strategy = models.CharField(
        max_length=64,
        unique=True,
        default='spot_scalping',
        help_text="Название стратегии (например, spot_scalping)"
    )
